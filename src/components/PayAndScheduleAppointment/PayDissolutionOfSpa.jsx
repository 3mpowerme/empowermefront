...
const PayDissolutionOfSpa = forwardRef(({ onComplete = () => {} }, ref) => {
  ...
  const ensureServiceOrder = async () => {
    const payload = {
      serviceCode: 'dissolution_of_spa',
      companyId,
      count,
    };
      ...
      <Switch value={view}>
        ...
        <Switch.Item case={PAY_APPOINTMENT_VIEW}>
          <div className="flex flex-col items-center space-y-5 px-6 md:px-10 lg:px-20 mt-10 md:mt-20">
            <h1 className="text-xl font-bold text-center">
              ¿Necesitas terminar una Sociedad por Acciones (SpA)?
            </h1>
            <p>
              Si creaste tu empresa en el Registro de Empresas y Sociedades o la migraste al mismo
              (Empresaen1día), podemos ayudarte en la Disolución Sociedad por Acciones (si la
              creaste en el sistema tradicional, en papel, no trabajamos con empresas que
              pertenezcan a ese registro para este servicio).
            </p>
            <div className="w-1/5">
        ...
        <Switch.Item case={SUCCESSFULL_SUBSCRIPTION_VIEW}>
          <SuccessfullPayment
            ...
            goTo={'legal_services/dissolution_of_spa'}
          />
        </Switch.Item>
      </Switch>
    </>
...
export default PayDissolutionOfSpa;

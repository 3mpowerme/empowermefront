...
const PayOrdinaryShareholderMeeting = forwardRef(({ onComplete = () => {} }, ref) => {
  ...
  const ensureServiceOrder = async () => {
    const payload = {
      serviceCode: 'ordinary_shareholders_meeting',
      companyId,
      count,
    };
      ...
      <Switch value={view}>
        ...
        <Switch.Item case={PAY_APPOINTMENT_VIEW}>
          <div className="flex flex-col items-center space-y-5 px-6 md:px-10 lg:px-20 mt-10 md:mt-20">
            <h1 className="text-xl font-bold text-center">
              ¿Tienes una Sociedad por Acciones (SpA) y necesitas redactar el acta de la junta
              ordinaria de accionistas?
            </h1>
            <p>
              La junta ordinaria de accionistas es OBLIGATORIA para todas las sociedades por
              acciones (aunque seas el único accionista) y se debe realizar a lo menos 1 vez al año
              si así lo establecen los estatutos de tu sociedad (y en casi todos los casos es así ya
              que se trabajan con estatutos tipo).
            </p>
            <p>
              Debe redactarse un acta de la misma, la que se debe protocolizar ante notario y no
              hacerla constituye un vicio legal que puede perjudicar a tu empresa.
            </p>
            <div className="w-1/5">
        ...
        <Switch.Item case={SUCCESSFULL_SUBSCRIPTION_VIEW}>
          <SuccessfullPayment
            ...
            goTo={'taxes_and_accounting/ordinary_shareholders_meeting'}
          />
        </Switch.Item>
      </Switch>
    </>
...
export default PayOrdinaryShareholderMeeting;
